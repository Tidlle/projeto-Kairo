...
| Dashboard financeiro (entradas/saídas, categorias, gasto médio, heatmap) | ✅ Edge Function `sync-finance` implantada e validada com dado real (653 transações, 20 categorias) |
...
- **Edge Functions**: `sync` (cron diário, sincroniza Pluggy → Postgres), `sync-routine` (chamada pelo app, sincroniza tarefas/hábitos/metas/agenda entre o SQLite local e o Postgres) e `sync-finance` (chamada pelo dashboard, espelha `transactions`/`categories` do Postgres pro SQLite local — só leitura, sem push).
