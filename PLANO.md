...
**O deploy ficou pendente por uma sessão, depois fechou.** O Supabase CLI não estava autenticado neste ambiente (`ProjectsListUnexpectedStatusError: Unauthorized`) — pedi pro usuário rodar `npx supabase login`, ele fez, e o deploy completou na mesma conversa. Validado com o mesmo rigor de sempre: sem segredo → `401`; com segredo → `200` e dado real — **653 transações, 20 categorias**, no formato exato que `SyncTransactionRow`/`SyncCategoryRow` esperavam, sem nenhum ajuste de última hora. Nenhuma linha de teste ficou pra trás desta vez: `sync-finance` é só leitura, então validar contra produção não escreve nada — diferente de `sync-routine`/`sync`, que sempre deixaram (e precisaram apagar depois) uma linha de teste.

**Estado final desta rodada:** 166 testes (145 anteriores + 21 novos). Type-check e lint limpos. Primeiro uso formal de plan mode nesta sessão, dado o tamanho do corte (schema novo, sincronização nova, biblioteca nova, tela nova). **Fase 1 do roadmap fechada** — dashboard financeiro implementado, testado e implantado, dado real fluindo do Postgres pro SQLite local.
