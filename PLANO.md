...
**Atualização — a lacuna acima foi fechada numa rodada bem posterior.** Com acesso de automação concedido desta vez, abri o `%LOCALAPPDATA%\Kairo` de verdade e vi a tela "Hoje" renderizando: Saldo R$ 292,72, as 3 prioridades, os 3 hábitos com os streaks certos, e a meta "Apartamento em Fortaleza" em 44% — os mesmos números que a inspeção do arquivo `.db` já tinha previsto, agora confirmados por observação direta, não por dedução. **A reatividade também foi testada de verdade**: tocar em "Cozinhar em vez de pedir" marcou o hábito e o streak subiu de 5 para 6 dias na hora, sem recarregar — a prova visual do `change-bus` que faltava. **Achado incidental**: esse instalador é anterior ao rollout da identidade visual (tema/tipografia/ícone) — mostra o ícone padrão do Tauri e o acento azul antigo, não a kappa/Âmbar. **Resolvido na mesma sessão**: `npm run build -w @kairo/desktop` de novo, mais `npx tauri icon` (o ícone do app mobile e o conjunto de ícones do Tauri são arquivos separados — o rebuild sozinho só pega o CONTEÚDO da janela, não o ícone do executável). Reinstalado e confirmado: kappa na barra de título e na barra de tarefas, tema novo renderizando.
...
**"Validado localmente" não bastou — o primeiro push quebrou o CI de verdade.** Rodar os mesmos comandos localmente antes de subir (como descrito acima) passou limpo, mas a primeira execução do workflow no GitHub falhou mesmo assim, no step de type-check do `apps/mobile`. Causa: `apps/mobile/expo-env.d.ts` — o arquivo que traz `/// <reference types="expo/types" />`, de onde vêm as declarações de tipo para `.css`/`.module.css` — está no `.gitignore` (o próprio conteúdo do arquivo diz "should be in your git ignore") porque o Expo CLI o gera sozinho na primeira vez que você roda `expo start` localmente. **Esse arquivo já existia no meu disco há semanas**, de uma das primeiras rodadas desta sessão — por isso `tsc` sempre passou localmente, inclusive num teste com `node_modules` totalmente limpo (`rm -rf` + `npm ci`), já que remover `node_modules` não apaga esse arquivo. Só um checkout LIMPO de verdade (o do runner do GitHub, que nunca rodou `expo start`) expõe a ausência dele.

**Diagnosticado reproduzindo o cenário exato, não advinhando.** Antes de mexer no workflow, testei duas hipóteses possíveis por eliminação: (1) `.expo/types/router.d.ts` ausente — removido e testado, `tsc` continuou passando limpo, hipótese descartada; (2) resíduo de instalações anteriores desta sessão longa mascarando um problema — testado com `rm -rf` de todo `node_modules` do monorepo seguido de `npm ci` de verdade, `tsc` ainda passava, hipótese descartada. Só ao remover fisicamente o `expo-env.d.ts` o erro reapareceu, idêntico em espécie ao que o CI reportou (`Cannot find module` para os `.css`). **Corrigido gerando o arquivo dentro do próprio workflow**, com o conteúdo estático exato que o Expo produziria — sem depender de adivinhar qual comando do Expo CLI dispara essa geração automaticamente (nenhum dos testados — `expo install --check`, `expo export` — recriava o arquivo).

**Um erro secundário no caminho, do meu próprio comando: rodei `expo lint --fix` da RAIZ do monorepo por engano, em vez de dentro de `apps/mobile`.** Isso fez a Expo CLI tentar fazer o mesmo bootstrap de ESLint na raiz — que não é um projeto Expo — criando um `eslint.config.js` quebrado ali e adicionando um script `"lint": "expo lint"` indevido ao `package.json` da raiz. Percebido pela mensagem de erro (o novo config na raiz falhava ao carregar) antes de commitar; revertido com `git checkout -- package.json` e apagando o arquivo espúrio — nada disso chegou a ser enviado ao GitHub.

**Estado final desta rodada:** 145 testes continuam passando, type-check limpo nos dois projetos (incluindo o cenário que o CI realmente exercita — checkout limpo, sem o `expo-env.d.ts` pré-existente), lint limpo (1 erro real de código antigo encontrado e resolvido). **Lição registrada:** "rodei os mesmos comandos localmente" não é o mesmo que "testei o que o CI testa" quando o ambiente local tem arquivos gerados que um checkout limpo nunca teria — vale sempre desconfiar de artefatos `.gitignore`d que também aparecem em `include` do `tsconfig.json`.

---

### Implementado: identidade visual — tema, tipografia e o ícone do app

> Tokens em [`apps/mobile/src/constants/theme.ts`](apps/mobile/src/constants/theme.ts) · tema forçado em [`use-color-scheme.ts`](apps/mobile/src/hooks/use-color-scheme.ts)/[`.web.ts`](apps/mobile/src/hooks/use-color-scheme.web.ts) · fontes em [`_layout.tsx`](apps/mobile/src/app/_layout.tsx) · estilo `money` em [`themed-text.tsx`](apps/mobile/src/components/themed-text.tsx) · ícone em [`apps/mobile/assets/images/`](apps/mobile/assets/images) e [`animated-icon.tsx`](apps/mobile/src/components/animated-icon.tsx).

**A paleta e a tipografia vieram de uma exploração visual, não de uma escolha única.** Pedido o nome "Kairo" já fechado, o processo foi: 4 direções tipográficas inteiras (Kairós, Norte, Cerrado, Sereno), a direção Kairós (Fraunces + Inter) escolhida, 6 variações de cor dentro dela (Âmbar, Ferrugem, Bordô, Petróleo, Verde-garrafa, Ardósia), Âmbar escolhida, e um pedido pontual de trocar o acento secundário da versão Ardósia por um hex específico (`#C97B1E`) — que **falhou o contraste WCAG AA** (branco sobre `#C97B1E` dá ~3,31:1, abaixo do mínimo de 4,5:1 para texto normal). Corrigido com uma tinta de tinta escura como `onAccent` em vez de branco, e um tom mais claro (`#E6A94E`) para o acento no escuro pelo mesmo motivo — o mesmo tipo de ajuste que a paleta Verde-garrafa já tinha precisado. **Tema escuro fixo, não "escuro por padrão"**, foi uma decisão explícita perguntada e confirmada: o app nunca deve seguir a preferência do sistema.

**Rollout na base de código real, não só protótipo.** `Colors.light`/`Colors.dark` em `theme.ts`, `useColorScheme()` (nativo e Web) sempre retornando `'dark'`, `useTheme()` simplificado para um único caminho, `ThemeProvider` do Expo Router usando `DarkTheme` direto (antes lia `react-native`'s `useColorScheme()` puro, independente do hook próprio do app — inconsistência que só apareceria se algum dia o tema voltasse a ser dinâmico). Fraunces (`display`, títulos e logotipo) e Inter (`body`, incluindo o estilo `money` com `fontVariant: ['tabular-nums']` para números alinharem em coluna) carregadas via `@expo-google-fonts/*`, com a splash retida (`preventAutoHideAsync`) até elas carregarem — sem isso o texto piscaria na fonte do sistema antes de trocar.

**Dois problemas de consistência, achados espalhando a paleta pelas telas, não previstos de antemão:** o azul antigo da marca (`#3c87f7`) estava hardcoded em 9+ lugares (`tasks-screen.tsx`, `habits-screen.tsx`, `agenda-screen.tsx`, `goals-screen.tsx`, `progress-bar.tsx`) em vez de vir do tema — todos trocados para `theme.accent`; e a cor de prioridade "média" das tarefas (`#f08c00`, laranja) ficava visualmente idêntica ao novo acento âmbar — a mesma cor não pode significar "prioridade média" e "isso é a marca/é clicável" ao mesmo tempo. Resolvido trocando "média" para o azul antigo, que sobrou livre depois da migração.

**Um pedido de ajuste fino depois do rollout: a fonte do número de Saldo.** O card "Saldo" usava o mesmo estilo `subtitle` (Fraunces) dos títulos — uma serifa decorativa em cima de um número dificulta a leitura rápida. Criado um estilo `money` dedicado (Inter Bold, dígitos tabulares, leve `letter-spacing` negativo), usado só onde o valor precisa ser lido rápido.

**A marca do ícone virou uma maratona de exploração — mais de dez rodadas de conceitos, cada uma rejeitada até a última.** Na ordem: monogramas geométricos (K, arco, crescente); depois, inspirado no traço à mão livre da Pierre (assistente financeiro) — não copiando o mascote dela, só o princípio de um ícone desenhado como rabisco de caneta em vez de forma geométrica perfeita — hourglass, nascer do sol, relógio de bolso; depois versões "mais elaboradas" com múltiplos elementos (constelação, porta entreaberta, bússola, onda); depois ícones ligados ao NOME em si (K com o ponto do logotipo, ligadura tipográfica "Ka", K esboçado); a virada veio ao puxar pro lado grego — a letra **kappa (κ)**, primeira letra de "Καιρός" (a raiz de "Kairo"), mais o meandro (chave grega), o topete e a balança da estátua antiga de Kairós (segurada "no fio da navalha", encaixando com finanças de quebra); um desvio para ideias de mascote (o próprio Kairós correndo, uma coruja, uma raposa), voltando em seguida para a kappa por pedido explícito. **Escolha final: a kappa original, desenhada à mão** (três traços — haste, diagonal, e uma diagonal com uma curva no final), e a versão itálica do logotipo "kairo" (Fraunces itálico) reservada para títulos de seção dentro do app, não para o ícone/tela de abertura.

**Os PNGs de verdade foram gerados por rasterização programática, sem navegador.** `npx resvg-cli <svg> <png> -w -h` (renderizador Rust via `resvg-js`) converteu os SVGs da kappa nos tamanhos e variantes que cada plataforma precisa: `icon.png` (1024×1024, fundo cheio — a máscara de cantos arredondados é aplicada pelo próprio sistema operacional, não deve vir pré-cortada na fonte), `android-icon-foreground/background/monochrome.png` (ícone adaptativo Android — glifo sem fundo, fundo sólido separado, e uma versão monocromática preta para o ícone temático do Android 13+), `favicon.png` e `splash-icon.png`. Removido o bundle de ícone "Liquid Glass" padrão do Expo (`assets/expo.icon`, formato novo do Xcode 26/iOS 18) em favor do `icon.png` simples também no iOS — evita depender de um formato que não dá para validar sem Xcode.

**De quebra, um pente-fino achou código morto do template original do `create-expo-app`, nunca limpo.** `AnimatedIcon` (nativo e Web, dentro de `animated-icon.tsx`/`.web.tsx`) não era importado em lugar nenhum do app — só `AnimatedSplashOverlay` é usado de fato em `_layout.tsx`. Removido junto com os assets que só existiam para alimentá-lo: `expo-logo.png`, `logo-glow.png`, `animated-icon.module.css`. A splash animada em JS agora mostra o glifo da kappa em vez do logo padrão do Expo.

**O bug do Metro travado em `expo/AppEntry.js` reapareceu, duas vezes, por causas idênticas.** A mesma classe de bug documentada na rodada de testes em dispositivo físico — processos `expo start` antigos sobrevivendo e corrompendo a resolução do bundler — voltou a acontecer tanto no meio do rollout de tema quanto depois da implementação do ícone. Diagnosticado do mesmo jeito as duas vezes (`Get-CimInstance Win32_Process` para achar o processo travado, matar, e subir de novo com `--clear`), sem precisar reinvestigar a causa raiz. **Padrão que vale lembrar de cara da próxima vez que esse erro específico aparecer**, em vez de investigar do zero.

**Validado por tipo/lint/teste, mas com uma ressalva honesta sobre o que dá para confirmar sem build nativo.** `tsc --noEmit` e `expo lint` limpos, 145 testes passando (mudança de marca não altera lógica, então a contagem não mudou). O ícone real na tela inicial do celular **não foi confirmado visualmente** — só aparece de verdade num build nativo (`eas build` ou `expo prebuild`), nunca no Expo Go, que usa o ícone dele mesmo. Só a splash em JS (que roda em qualquer runtime, incluindo Expo Go) foi possível confirmar sem esse build.

**Estado final desta rodada:** 145 testes (sem mudança — troca de identidade visual, não de lógica). Type-check e lint limpos. Dois commits enviados ao GitHub (`d63ddf2` tema/tipografia, `2c0cdde` ícone + limpeza), CI verde nos dois.

---

### Implementado: build iOS instalável sem conta Apple Developer paga

> Workflow em [`.github/workflows/ios-unsigned-ipa.yml`](.github/workflows/ios-unsigned-ipa.yml).

**O pedido original era usar o EAS — a investigação mostrou que EAS não resolve isso de graça, e não é limitação do Expo.** Instalar um build de verdade (fora da App Store) num iPhone físico sempre passa por assinatura de código, e a Apple só libera a API do Developer Portal — que o EAS usa pra gerenciar certificados automaticamente — para quem tem o Program pago (US$ 99/ano). Confirmado pesquisando a fundo (não por suposição): builds de simulador não exigem conta paga, mas só rodam num Mac; builds locais (`eas build --local`) também não exigem conta Expo, mas ainda compilam via Xcode — precisam de um Mac de qualquer forma; e o caminho "conta Apple gratuita + Xcode local" existe, mas expira em 7 dias e também exige Mac.

**A saída real veio de separar duas coisas que pareciam a mesma: "compilar o app" e "assinar o app".** `expo prebuild` (comando do próprio Expo, não do EAS) gera o projeto nativo `ios/` de graça. Compilar esse projeto SEM assinatura nenhuma (`CODE_SIGNING_ALLOWED=NO`) é só um `xcodebuild archive` comum — não precisa de conta Apple nenhuma, só precisa rodar em macOS. E como o repositório é público, o GitHub dá **runners macOS de graça, sem limite de minutos** — a peça que faltava pra fechar o círculo sem precisar de Mac próprio. A assinatura em si fica pro **Sideloadly**, que resigna o `.ipa` já compilado com um Apple ID gratuito na hora de instalar — daí o app rodar por 7 dias antes de precisar reinstalar (limite do Apple ID grátis, não do processo de build).

**Dois bugs reais no primeiro workflow, achados só rodando de verdade — e um deles só diagnosticado com o log em mãos.** O primeiro run falhou em 8 segundos, cedo demais pra ser erro de compilação. Tentei recuperar o log exato por várias vias — API de logs (exige token, `403` mesmo em repositório público), anotações do commit (só devolve "exit code 66", sem a mensagem real), forçar o elemento `<details>` do visualizador a abrir via JavaScript (abriu, mas o conteúdo real vem por um canal ao vivo tipo WebSocket, não por um endpoint estático) — nenhuma bateu. Corrigi as duas causas mais prováveis para uma falha tão rápida (faltava `-destination` explícito, que o Xcode recente exige pra `archive`; e o *scheme* era descoberto via `xcodebuild -list -json`, que pode pegar um scheme de Pod em vez do app) e pedi o log ao usuário. **O log real revelou a causa verdadeira, diferente da minha suposição**: `ls *.xcworkspace` sem a flag `-d` lista o CONTEÚDO do pacote (que é uma pasta), não o nome dele — capturava `contents.xcworkspacedata` (o único arquivo lá dentro) como se fosse o workspace inteiro. Mesmo cuidado que a linha do *scheme* já tinha (`ls -d *.xcodeproj`), só não repliquei pro workspace na primeira versão. **Lição prática confirmada de novo nesta sessão**: quando a causa raiz não é óbvia pela suposição mais provável, o log de verdade resolve mais rápido que mais uma rodada de tentativa e erro — vale pedir cedo, não como último recurso.

**Instalado e confirmado num iPhone físico de verdade**, via Sideloadly com Apple ID gratuito. Faltava mais um degrau que não estava no roteiro original: o iOS (desde a versão 16) exige **"Modo de Desenvolvedor"** ativado explicitamente em Ajustes → Privacidade e Segurança para rodar qualquer app instalado fora da App Store/TestFlight — separado do passo de "confiar no desenvolvedor" já conhecido. Depois de ativado (com reinício do aparelho), o app abriu normalmente — **primeira confirmação visual do ícone real da kappa na tela inicial de um iPhone**, e o primeiro ambiente onde `expo-notifications` pode ser testado de verdade (Expo Go nunca suportou isso completamente desde o SDK 53).

**Decisão consciente de escopo: o build não recebeu as variáveis de sincronização.** `EXPO_PUBLIC_SYNC_FUNCTION_URL`/`EXPO_PUBLIC_MOBILE_SYNC_SECRET` não foram passadas como secrets do GitHub Actions — o app funciona inteiro com o banco local, só o botão "Sincronizar" da tela Hoje fica ausente (mesmo comportamento condicional já documentado: o botão só aparece se as duas variáveis estiverem presentes). Adicionar isso ao workflow fica para quando for realmente necessário testar a sincronização num build nativo.

**Estado final desta rodada:** nenhum teste novo (infraestrutura de build/distribuição, sem lógica de negócio). Três commits enviados ao GitHub (workflow inicial + dois fixes). **Primeira instalação bem-sucedida de um build nativo do Kairo fora do Expo Go, em qualquer plataforma.**

---

### Implementado: dashboard financeiro — fecha a Fase 1

> Protocolo em [`packages/core/src/finance-sync-protocol.ts`](packages/core/src/finance-sync-protocol.ts) · agregações em [`packages/core/src/finance-dashboard.ts`](packages/core/src/finance-dashboard.ts) · Postgres em [`packages/db/src/finance-sync.ts`](packages/db/src/finance-sync.ts) · Edge Function em [`supabase/functions/sync-finance/`](supabase/functions/sync-finance) · motor local em [`apps/mobile/src/db/finance-sync.ts`](apps/mobile/src/db/finance-sync.ts) · gráficos em [`apps/mobile/src/components/kairo/charts/`](apps/mobile/src/components/kairo/charts) · tela em [`apps/mobile/src/components/screens/dashboard-screen.tsx`](apps/mobile/src/components/screens/dashboard-screen.tsx).

**O usuário pediu pra focar em produto — e a investigação revelou que "fazer o dashboard" não era uma tarefa de UI.** Antes de desenhar qualquer gráfico, dois agentes (um de exploração, um de design) confirmaram um buraco estrutural: o SQLite local nunca teve uma tabela de transações. "Saldo" na tela Hoje sempre foi `SUM(accounts.balanceCents)` — um número achatado, nunca derivado de um ledger. O ledger de verdade (Pluggy → Postgres) vive só no servidor, sincronizado por um pipeline totalmente separado que nunca toca o app. Sem isso investigado primeiro, o dashboard teria sido construído em cima de dado que não existe localmente.

**Três decisões de arquitetura, confirmadas com o usuário antes de escrever qualquer código** (via plan mode — pela primeira vez nesta sessão o fluxo formal de planejamento foi usado, dado o tamanho do corte):
1. **Espelho local, pull-only, sem extensão do `sync-routine`.** `sync-protocol.ts`/`sync-routine` documentam seu próprio escopo como "o que o usuário edita", com last-write-wins bidirecional — peso morto para dados que o cliente nunca escreve. Caminho novo e paralelo: `finance-sync-protocol.ts` (tipos, sem `isNewer`), `pullFinance` (Postgres), Edge Function `sync-finance` (mesmo segredo `MOBILE_SYNC_SECRET` de `sync-routine` — o raio de confiança é idêntico, e continua só-leitura), `syncFinance` (motor local). O servidor é sempre a única fonte da verdade: `applyFinancePulled` sobrescreve sem checar `updated_at` local, porque não há edição local pra proteger.
2. **Espelho completo, sem janela de meses.** Puxa tudo que mudou desde o watermark, sem limite de data — centenas de linhas não pesam num SQLite local, e evita lógica de poda dos dois lados.
3. **Gráficos à mão com `react-native-svg`, não Victory Native XL.** O plano original (seção 5) previa "Victory Native XL (mobile) / Recharts (web)" — duas bibliotecas diferentes pra três runtimes com UMA base de código. Skia (dependência do Victory Native XL) tem comportamento incerto dentro do webview do Tauri. `react-native-svg` — instalado agora como dependência direta, antes só transitiva — dá uma implementação só, funcionando igual em iOS/Android/Web/Tauri, no mesmo espírito do `ProgressBar` já existente.

**Regra de exclusão extraída pra um único lugar.** `summarize()` (pluggy/map.ts, usado na ingestão) e as agregações novas do dashboard precisavam da MESMA condição — "não é transferência interna, já foi confirmada (`POSTED`)". Em vez de duplicar, virou `isRealSpend()`, exportada e reutilizada nos dois lugares — a mesma disciplina de "uma verdade, um lugar" que `isNewer()` já estabeleceu para sincronização.

**Paleta escolhida com a skill `dataviz` carregada antes de desenhar qualquer componente**, mesmo sem poder rodar o validador da skill (feito pra CSS/web, não React Native): entradas x saídas e o heatmap usam azul/laranja, não verde/vermelho — o par mais comum de confundir em daltonismo (proto/deuteranopia) — reaproveitando o MESMO par nos dois gráficos pra uma linguagem visual consistente ("pra onde o dinheiro foi" é sempre a mesma cor). O donut de categorias usa uma ordem fixa de 6 cores (âmbar primeiro, é a cor da marca) com o resto agrupado em "Outros" — nunca uma 9ª cor gerada na hora, que não teria como continuar distinguível.

**Um bug do React Compiler, não de lógica.** A primeira versão do donut mutava uma variável `angle` de fora dentro de um `.map()` — o React Compiler (ativado neste projeto) rejeita isso: "Cannot reassign variable after render completes". Corrigido com `reduce` puro, carregando o ângulo acumulado no próprio acumulador em vez de uma variável externa.

**Entrada na navegação reaproveitando um padrão já existente, não um novo.** Em vez de embrulhar o card "Saldo" inteiro num `Pressable` novo (como o plano original sugeria), o `SectionCard` já tinha `actionLabel`/`onPressAction` — o mesmo mecanismo que Agenda/Prioridades/Hábitos/Meta já usam. Consistência venceu a sugestão original.

**Testado com o mesmo rigor de sempre — real em cada camada, nunca mock.** `packages/core/test/finance-dashboard.test.ts` (13 testes, puro, incluindo os casos de exclusão e a virada de ano nos ranges padrão); `packages/db/test/finance-sync.test.ts` (`pullFinance` contra PGlite real, isolamento por usuário, watermark); `apps/mobile/test/finance-sync.test.ts` (`syncFinance` contra `better-sqlite3` + servidor `node:http` de verdade, mesmo padrão de `sync.test.ts`). Um teste antigo (`db.test.ts`, "cria as 8 tabelas esperadas") precisou de ajuste — virou 10, `categories` e `transactions` somadas.

**O que ficou pendente, e por quê.** O deploy da Edge Function `sync-finance` não rodou: o Supabase CLI não está autenticado neste ambiente (`ProjectsListUnexpectedStatusError: Unauthorized`), diferente de rodadas anteriores onde uma sessão já vinha logada. Código e testes provam o contrato dos dois lados isoladamente (Postgres via PGlite, app via servidor HTTP de mentira) — falta só a chamada real de ponta a ponta contra produção, que todo deploy anterior deste projeto sempre fez antes de considerar uma Edge Function pronta. `EXPO_PUBLIC_FINANCE_SYNC_FUNCTION_URL` já foi adicionada ao `.env` local, apontando pra URL que a função vai ocupar — falta só o deploy em si.

**Estado final desta rodada:** 166 testes (145 anteriores + 21 novos). Type-check e lint limpos. Primeiro uso formal de plan mode nesta sessão, dado o tamanho do corte (schema novo, sincronização nova, biblioteca nova, tela nova). **Fase 1 do roadmap fecha assim que o deploy acontecer** — o código já está pronto e testado, só falta essa última chamada.
...
- ~~Skill de README, repositório git e CI~~ ✅ skill `update-readme` documentando como manter o README (referência prática) separado do PLANO.md (narrativa); primeiro commit do projeto (168 arquivos) e push para [github.com/Tidlle/projeto-Kairo](https://github.com/Tidlle/projeto-Kairo); GitHub Actions rodando type-check + lint + testes a cada push/PR — **confirmado passando de verdade no runner do GitHub** (não só localmente: a primeira tentativa falhou por causa de `expo-env.d.ts` ausente num checkout limpo, achado e corrigido, segunda execução verde); ESLint configurado pela primeira vez, achou e corrigiu 1 problema real de código antigo
- ~~Identidade visual (tema, tipografia, ícone)~~ ✅ tema escuro fixo, Fraunces/Inter, paleta Âmbar aplicados em toda a base de código; depois de mais de dez rodadas de exploração de ícone, a letra grega kappa (κ) — raiz de "Καιρός" — escolhida e implementada como ícone real do app (gerado via `resvg-cli`, sem navegador); logotipo "kairo" em itálico reservado para títulos internos; código morto do template Expo (`AnimatedIcon`, `expo-logo.png`, `logo-glow.png`) removido no processo
- ~~Build iOS instalável sem conta Apple Developer paga~~ ✅ `expo prebuild` + `xcodebuild` sem assinatura num runner macOS grátis do GitHub Actions, `.ipa` resignado com Apple ID gratuito via Sideloadly — dois bugs de shell corrigidos (`ls` sem `-d` em pasta `.xcworkspace`, descoberta de *scheme* pouco confiável) e o Modo de Desenvolvedor do iOS 16+ identificado como passo extra necessário. **Primeira instalação de um build nativo do Kairo fora do Expo Go**, com o ícone da kappa confirmado na tela inicial de um iPhone real
- ~~Rebuild do app desktop com a identidade visual nova~~ ✅ `npm run build -w @kairo/desktop` de novo, mais `tauri icon` pra regenerar o conjunto próprio de ícones do Tauri (separado do ícone do app mobile — o rebuild sozinho não pega isso). Confirmado abrindo o app reinstalado: ícone da kappa na barra de título e na barra de tarefas, tema/cores novos renderizando (prioridade "média" já em azul, não mais laranja)
- ~~Dashboard financeiro (fecha a Fase 1)~~ ✅ espelho local pull-only de `transactions`/`categories` (protocolo, motor Postgres, Edge Function `sync-finance`, motor local — tudo novo, paralelo ao `sync-routine`), 4 agregações puras testadas, gráficos em `react-native-svg` desenhados com a skill `dataviz` (paleta azul/laranja pra evitar o par vermelho/verde de daltonismo). 166 testes no total. **Falta só o deploy da Edge Function** — o CLI do Supabase não está autenticado neste ambiente
